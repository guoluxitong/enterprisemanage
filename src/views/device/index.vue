      ...
      handleselect(){
        ...
        queryDeviceOnline().then(data => {
          ...
          for(var i=0;i< data.data.data.length;i++){
            for(var j=0;j< this.deviceControllerNo.length;j++){
              if(data.data.data[i].deviceSuffix.substring(0, this.deviceControllerNo[j].length)== this.deviceControllerNo[j]){
       ...
       replacepos(text,start,replacetext)
  {
    ...
    return mystr;
      ...
      inint() {
        ...
        queryDeviceOnline().then(data => {
          this.devicesList=[]
          console.log(this.deviceControllerNo)
          for(var i=0;i< data.data.data.length;i++){
            for(var j=0;j< this.deviceControllerNo.length;j++){
              if(data.data.data[i].deviceSuffix.substring(0, this.deviceControllerNo[j].length)== this.deviceControllerNo[j]){
                this.devicesList.push(data.data.data[i])
          ...
          queryDeviceByOnline().then(requst => {
            ...
            queryWeChatUser().then(data => {
              ...
              for(var i=0;i< data.data.data.length;i++){
                for(var j=0;j< this.deviceControllerNo.length;j++){
                  if(data.data.data[i].substring(0, this.deviceControllerNo[j].length)== this.deviceControllerNo[j]){
